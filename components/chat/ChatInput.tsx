...
import { Send, Sparkles, ChevronDown, Square, Check } from 'lucide-react';
  ...
  return (
    <div className={cn('border-t border-border bg-background-primary p-4', className)}>
      <div className="mx-auto max-w-3xl">
        <div
          className={cn(
            'relative rounded-xl border border-border bg-background-secondary transition-all duration-200',
        ...
        >
          ...
          <textarea
            ...
            rows={1}
            className="w-full resize-none rounded-xl bg-transparent px-4 py-3 text-text-primary placeholder:text-text-placeholder focus:outline-none"
          ...
          <div className="flex items-center justify-between border-t border-border-subtle px-3 py-2">
            ...
            <div className="relative" ref={dropdownRef}>
              <button
                ...
                disabled={disabled || isGenerating}
                className={cn(
                  'flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-sm transition-colors hover:bg-background-tertiary disabled:opacity-50',
                  modelDisplay.color
                )}
              >
                {selectedModel === 'auto' ? (
              ...
              {/* Dropdown Menu */}
              {isModelOpen && (
                <div className="absolute bottom-full left-0 z-50 mb-2 w-56 animate-fade-in rounded-lg border border-border bg-background-secondary p-1 shadow-lg">
            ...
            {isGenerating ? (
              <button
                onClick={onStop}
                className="flex h-8 w-8 items-center justify-center rounded-lg bg-error text-white transition-all hover:bg-red-600"
